...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
    ...
    /**
     * @ORM\OneToMany(targetEntity=Readings::class, mappedBy="user_id")
     */
    private $readings;

    /**
     * @ORM\OneToMany(targetEntity=Crush::class, mappedBy="user_id")
     */
    private $crushes;
    ...
    public function removeReading(Readings $reading): self
    {
        if ($this->readings->removeElement($reading)) {
            // set the owning side to null (unless already changed)
            if ($reading->getUserId() === $this) {
                $reading->setUserId(null);
            }
        }
    ...
    public function removeCrush(Crush $crush): self
    {
        if ($this->crushes->removeElement($crush)) {
            // set the owning side to null (unless already changed)
            if ($crush->getUserId() === $this) {
                $crush->setUserId(null);
            }
        }
